...
namespace MobileService.Tests.Api.StatisticsTests
{
    ...
    public class GetUserDailyStatsTests : IClassFixture<CustomWebApplicationFactory<Startup>>
    {
        private readonly HttpClient _client;

        public GetUserDailyStatsTests(CustomWebApplicationFactory<Startup> factory)
        {
        ...
        public async Task TestA()
        {
            ...
            var response = await _client.GetAsync("api/statistics/getuserdailystats");

            Assert.Equal(HttpStatusCode.OK, response.StatusCode);

            var output = JsonConvert.DeserializeObject<StatsUserDailyGetModel>(await response.Content.ReadAsStringAsync());

            Assert.Equal(6, output.TotalFlashcards);
            Assert.Equal(3, output.ToLearnFlashcards);
